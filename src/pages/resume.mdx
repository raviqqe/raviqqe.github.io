---
layout: ../layouts/Resume.astro
title: Yota Toyama
description: A software engineer and programming language enthusiast. Knowledgeable in Rust, C++, Go, and TypeScript.
---

import Print from "../components/Print.astro";

# Yota Toyama

[![GitHub](https://img.shields.io/badge/github-raviqqe-red.svg?style=flat-square)](https://github.com/raviqqe)
[![LinkedIn](https://img.shields.io/badge/linkedin-yota_toyama-blue.svg?style=flat-square)](https://www.linkedin.com/in/yota-toyama-29b313bb/)
[![Portfolio](https://img.shields.io/badge/website-raviqqe.com-silver.svg?style=flat-square)](https://raviqqe.com)

A software engineer equipped with production-level knowledge about Rust, C++, Go, Scala, TypeScript, Node.js, low-layer programming, and [Clean Architecture](https://blog.cleancoder.com/uncle-bob/2012/08/13/the-clean-architecture.html). Enthusiastic about programming languages and their implementations.

<Print>A web version of this resume is at https://raviqqe.com/resume/.</Print>

- Email: ytoyama42@gmail.com

## Work Experience

### [Mapbox](https://mapbox.com/)

- **Period**: November 2023 - Present, Japan
- **Position**: Software Engineer II (full time)
- **Technologies**: C++, Rust, TypeScript, Amazon Web Services, Docker, HTTP, GRPC, Protocol Buffer, React.js
- Development of the navigation system for electric vehicles.
  - Development of support for battery-swapping electric vehicles.
- Development of the navigation system for general vehicles.
  - Development of the lane navigation component
  - Halved metrics of dangerous lane changes
  - Developed on global optimization of suggested lanes over routes
  - End-to-end development of the lane accessibility feature
- Worked on migration from monolithic to microservice architecture for the system component

### [Spare Labs](https://spare.com/)

- **Period**: September 2018 - December 2022 (4 years), Canada
- **Position**: Algorithm Engineer, Full Stack Engineer, and Site Reliability Engineer (full time)
- **Technologies**: Scala, TypeScript, Kubernetes, Google Cloud Platform, Cloudflare, Docker, HTTP, PostgreSQL, Redis, Prometheus, Grafana, React.js, React Native
- Development of an on-demand transit system.
  - Including RESTful API and rider-driver matching backend systems, an admin web application, and rider/driver mobile applications.
- Development of rider-driver matching systems.
- Development of a traffic information system and its integration with a routing system.
- Efficiency and quality optimization of rider-driver matching systems.
- Design and implementation of driver navigation API integrated with a routing system.
- Optimization of relational database queries on PostgreSQL.
- Design and implementation of mass transit integration and whole trip planning using [General Transit Feed Specification (GTFS)](https://developers.google.com/transit/gtfs).
- Design and implementation of zero-downtime PostgreSQL data migration across different cloud vendors.
- Development of GitOps pipelines using GitHub, ArgoCD and Kubernetes.
- Design and implementation of a Prometheus/Grafana telemetry stack on a Kubernetes cluster.
- Pipeline and task automation for an infrastructure stack of Kubernetes, Helm, Terraform, and Google Cloud.
- Monolith-to-microservice migration of a RESTful API service.
- Scala, TypeScript, Node.js, React, React Native, HashiCorp Configuration Language, and shell scripts are used throughout projects listed above.

### Vernax

- **Period**: May 2017 - August 2017 (4 months), Malaysia
- **Position**: Full Stack Engineer (full time)
- MVP implementation of a personal assistant application that completes tasks, such as posting on social media and requesting Uber rides on behalf of users through voice interfaces.
- Development of Deep Learning models of language model and voice recognition, Python, [TensorFlow][tensorflow], TypeScript, React Native, [GraphQL][graphql], Firebase, and Google Cloud Platform.

### Toyota Central R&D Labs.

- **Period**: March 2015 - April 2015 (2 months), Japan
- **Position**: Intern
- Implementation of a cutting machine simulator in MATLAB from scratch working with a mechanical engineer specialized in cutting engineering.
- Analysis on smoothness of cut surfaces with combinations of certain materials and cutting edges using the simulator.

## Open Source Projects

### [Pen][pen] (Rust)

- The functional programming language inspired by [Go](https://go.dev/)'s philosophy.
- Automatic thread-safe memory management by [the Perceus reference counting algorithm](https://www.microsoft.com/en-us/research/uploads/prod/2020/11/perceus-tr-v1.pdf).
  - ["How to Implement the Perceus Reference Counting Garbage Collection"](https://hackernoon.com/how-to-implement-the-perceus-reference-counting-garbage-collection) on Hackernoon.
- Foreign Function Interface (FFI) against Rust supporting both non-`async` and `async` functions using proc macros and the C ABI.

### [Stak](https://github.com/raviqqe/stak) (Rust, Scheme)

- A Scheme virtual machine and ahead-of-time bytecode compiler based on [Ribbit Scheme](https://github.com/udem-dlteam/ribbit/tree/main).
- No `alloc` and no `std`.
- Designed to run on a machine stack as well as on heap.
  - Small memory footprints (~ a few kilobytes.)
- Performance comparable with [Gambit Scheme](http://gambitscheme.org/).

### [Schemat][schemat] (Rust)

- The blazingly fast Scheme code formatter.
- Built with [the new allocator API](https://doc.rust-lang.org/std/alloc/trait.Allocator.html) (available only in nightly Rust as of August 2023) for arena memory allocation in parsing and formatting algorithms.

### [Turtle](https://github.com/raviqqe/turtle-build) (Rust)

- A [Ninja][ninja]-compatible build system for high-level programming languages.
- Asynchronous task execution and scheduling integrated with [Tokio][tokio] runtime.
- Performance comparable with [Ninja][ninja] (0.4 time slower) despite of compact implementation.

### [F\-\-](https://github.com/raviqqe/fmm) (Rust)

- Compiler infrastructure and its intermediate language for functional programming languages inspired by [C-- in Glasgow Haskell Compiler](https://gitlab.haskell.org/ghc/ghc/-/wikis/commentary/rts/cmm).
- Supports compilation of the intermediate language into both [LLVM IR][llvm] and C.

### [Melior](https://github.com/raviqqe/melior) (Rust)

- The rustic [MLIR](https://mlir.llvm.org/) bindings for Rust.
- Automatic API generation by proc macros and [TableGen](https://llvm.org/docs/TableGen/) (WIP.)

### [Muffet](https://github.com/raviqqe/muffet) (Go)

- The fast website link checker.
- Parallel check of URL liveness using goroutines and channels.

For other projects, see [my portfolio page](/) at [raviqqe.com](/).

## Education

### Toyota Technological Institute

- **Period**: April 2016 - April 2017
- **Degree**: Studied towards M.A. in Computer Science (Not Completed)
- Further research on Deep Learning application in text classification.
  - ["Utilizing Visual Forms of Japanese Characters for Neural Review Classification"](https://aclanthology.org/I17-2064/) by Yota Toyama, Makoto Miwa, Yutaka Sasaki.
- Research on question answering by Deep Learning at TTI At Chicago for 3 months.

### Toyota Technological Institute

- **Period**: April 2012 - March 2016
- **Degree**: B.A. in Mechanical Engineering and Computer Science
- Research on applying Deep Learning to text classification for languages with ideograms (e.g. Kanjis in Japanese) combining convolutional and recurrent neural networks.
- Implementation and development of Deep Learning models using Python, [TensorFlow][tensorflow], and [Chainer](https://chainer.org/).
- Development of data processing/analysis pipelines for Deep Learning models.

## Technology Stack

### Languages

- Rust
- C++
- Go
- Scala
- TypeScript / JavaScript
- Python
- Shell script

### Frameworks / Runtimes

- TypeScript / JavaScript
  - Node.js
  - React / React Native
  - Remix.js / React Router
  - Next.js
- Rust
  - Tokio
- RESTful API
- GRPC
- GraphQL

### Databases

- PostgreSQL
- Redis
- Prometheus
- Grafana

### Platforms

- Kubernetes
- Helm
- Docker
- Terraform
- Amazon Web Services
- Google Cloud Platform
- Cloudflare
- Vercel
- Firebase

[graphql]: https://graphql.org/
[llvm]: https://llvm.org/
[ninja]: https://ninja-build.org/
[pen]: https://github.com/pen-lang/pen
[schemat]: https://github.com/raviqqe/schemat
[tensorflow]: https://www.tensorflow.org/
[tokio]: https://github.com/tokio-rs/tokio
